...
export default function ChefDashboardPage() {
  const { loading: productsLoading, data: products } =
    useRequest(getDashboardProducts);
  const { loading: ordersLoading, data: orders } =
    useRequest(getDashboardOrders);
  ...
  const totalRevenue = Math.round(
    orders.reduce((total, order) => total + order.total * order.quantity, 0) ||
      0
  );
  ...
  const ordersData = Object.entries(ordersGroupedByDate).map(
    ([date, orders]) => ({
      date,
  ...
  const productData = Object.entries(productsGroupedByStatus).map(
    ([status, products]) => ({
      name: status,
